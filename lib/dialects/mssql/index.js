...
class Client_MSSQL extends Client {
  ...
  prepBindings(bindings) {
    return map(bindings, (value) => {
      if (Array.isArray(value)) { // MTRK
        if (value.every((val, i, arr) => val === arr[0])) return value[0];
      }
      return value;
    });
  }
  ...
  _query(connection, query) {
    return new Promise((resolve, reject) => {
      ...
      this._assignBindings(request, query.bindings);
      // dumpObject('MTRK914OK ', query.bindings, 0);
      this._enqueueRequest(request, connection);
    });
  }
    ...
    response = response.map((row) =>
      row.reduce((columns, r) => {
        ...
        if (columns[colName]) {
          ...
          // columns[colName].push(r.value); // MTRK This caused duplication bug e.g. [18,18] instead of 18
        } else {
          columns[colName] = r.value;
        }
