...
class TableCompiler_MSSQL extends TableCompiler {
  ...
  alterColumns(columns, colBuilder) {
    ...
    columns.sql.forEach((sql) => {
      // TDOTOD [id] int identity(1,1) not null primary key
      this.pushQuery({
        sql:
          (this.lowerCase ? 'alter table ' : 'ALTER TABLE ') +
          this.tableName() +
  ...
  dropColumn() {
    ...
    for (const column of columns) {
      const baseQuery = `
              ...
              IF @constraint IS NOT NULL EXEC('ALTER TABLE ${this.tableNameRaw} DROP CONSTRAINT IF EXISTS ' + @constraint)`;
      this.pushQuery(baseQuery);
    }
    this.pushQuery(
      (this.lowerCase ? 'alter table ' : 'ALTER TABLE ') +
  ...
  createFKRefs(runner, refs) {
    ...
    return Promise.all(
      refs.map(function (ref) {
        ...
        return runner.query({
          sql:
            `SELECT ${keyName} FROM TABLE ${tableName}`
        });
        // return runner.query({
        //   sql:
