...
class Builder extends EventEmitter {
  ...
  where(column, operator, value) {
    ...
    dumpObject('MTRKwhere2Operator', operator, 0); // MTRK
    dumpObject('MTRKwhere2Value', value, 0); // MTRK
    if (value === undefined) { // MTRK
      // const debug = require('debug')('metrik'); // MTRK
      // debug(`MTRKundef COLUMN ${column}`); // MTRK
      value = null;
    }
    ...
    if (column === false || column === true) {
      // const debug = require('debug')('metrik'); // MTRK
      // debug(`MTRK12 ${args}`);
      return this.where(1, '=', column ? 1 : 0); // MTRK
    }
    ...
    if (argsLength === 2) {
      ...
      if ((value === null) || (value === undefined)) { // MTRK
